...
namespace OwnLang.ast
{
    public class Parser
    {
        ...
        private Statement assignmentStatement()
        {
            ...
            if (current.getType() == TokenType.TYPEDEF && get(1).getType() == TokenType.CONSTANT)
            {
                ...
                string variable = consume(TokenType.WORD).getText();
                consume(TokenType.EQ);
                AssignmentStatement assignmentStatement = new AssignmentStatement(variable, expression(), true, true);
                switch (type)
                {
                    case "int":
                        {
            ...
            if (current.getType() == TokenType.USING)
            {
                ...
                Statement stat = statementOrBlock();
                return new UsingStatement(stat, name, expr);
            }
            ...
            if (current.getType() == TokenType.CONSTANT)
            {
                ...
                string variable = consume(TokenType.WORD).getText();
                consume(TokenType.EQ);
                return new AssignmentStatement(variable, expression(), false, true);
            }
            ...
            if (current.getType() == TokenType.WORD && get(1).getType() == TokenType.INST_ENUM)
            {
                ...
                return new InstStatement(enums, enumf, fdelete);
            }
        ...
        private IfThenElseExpression ifThenElse()
        {
            ...
            return new IfThenElseExpression(condition, ifStatement, elseStatement);
        }
